...
public class BulletinPaieB extends BulletinPaieC implements Serializable {
		...
		if (this.cotisation != null) {
			...
			if (!this.isCotSelected) {
				...
				if (!this.isCotExist) {
					...
					HelperC.afficherAttention("ATTENTION", "Cette cotisation est déjà ajoutée!");
				}
			} else {
		...
		if (this.cmission != null) {
			...
			if (!this.isComSelected) {
				...
				if (!this.isComExist) {
					...
					HelperC.afficherAttention("ATTENTION", "Cette commission est déjà ajoutée!");
				}
			} else {
		...
		if (this.prime != null) {
			...
			if (!this.isPrmSelected) {
				...
				if (!this.isPrmExist) {
					...
					HelperC.afficherAttention("ATTENTION", "Cette prime est déjà ajoutée!");
				}
			} else {
		...
		if (this.deduction != null) {
			...
			if (!this.isDedSelected) {
				...
				if (!this.isDedExist) {
					...
					HelperC.afficherAttention("ATTENTION", "Cette retenu est déjà ajoutée!");
				}
			} else {
	...
	public void chargementElements() {
		...
		if (getDatePaie().before(this.exercice.getDateDebut()) || getDatePaie().after(this.exercice.getDateFin())) {
			...
			if (this.employe.getDateEntre().before(getDatePaie())) {
				...
				HelperC.afficherAttention("ATTENTION",
						"La date début du contrat de cet employé est supérieur é la date du paiement !");

				this.infoMsg = "La date début du contrat de cet employé est supé1rieur é la date du paiement !";
	...
	public void save() {
		...
		if (getTotalNetPay() >= this.montantNetMin) {
			...
			HelperC.afficherAttention("ATTENTION", "La paie ne peut pas étre inférieure é "
