   ...
   public void generationBulletin() throws InterruptedException {
     try {
       ...
       if (getDatePaie().before(this.exercice.getDateDebut()) || getDatePaie().after(this.exercice.getDateFin())) {
         ...
         HelperC.afficherAttention("ATTENTION", "La date de la paie ne doit pas�tre en d�hors de la p�riode de l'exercice !");
 		   disableSave=true;
         ...
         if (this.listEmploye != null && this.listEmploye.size() > 0)
         {
           ...
           for (EmployeC employe : this.listEmploye) {
             ...
             this.nomPrenom = String.valueOf(i) + " sur " + this.listEmploye.size() + " employ�s";
           ...
           HelperC.afficherMessage("Information", "Op�ration termin�e !");
         }
       
       } else {
         
         HelperC.afficherMessage("Information", "Il faut pr�ciser le mois et la date!");
         this.progressValue = 0;
       }
     
     } catch (Exception e) {
       
       HelperC.afficherMessage("ERREUR", "Echec de l'op�ration ! :" + e.getMessage(), FacesMessage.SEVERITY_ERROR);
     } 
   }
...
private void calculValeurComission(BulletinComissionC com,BulletinPaieC bulltinPaie) {
		     ...
		     if (listCredit != null && listCredit.size() > 0)
		      {
				for (CreditDetailC crd : listCredit) 
				{
					...
					detailCredit.setLibelle("Cr�dit : "+crdi.getBanque().getCode());
   ...
   public void onComplete() {
     FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("Op�ration termin�e !"));
   }
