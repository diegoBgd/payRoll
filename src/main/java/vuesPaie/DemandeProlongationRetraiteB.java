...
@ManagedBean
@ViewScoped
public class DemandeProlongationRetraiteB extends DemandeProlongationRetraiteC {
	...
	private String code, msgInfo, emplName;
	...
	private void completeEmploye() {
		if (selection != null) {
			Base fx = FichierBaseDAO.getInstance().getBaseById(selection.getIdFnctn(),
					Tables.getTableName(Tables.TableName.fonction));
			Base nv = FichierBaseDAO.getInstance().getBaseById(selection.getIdNvFormt(),
					Tables.getTableName(Tables.TableName.niveauFormation));
			Base catg = FichierBaseDAO.getInstance().getBaseById(selection.getIdCatgrie(),
					Tables.getTableName(Tables.TableName.categoriePersonnel));
			Base pers = FichierBaseDAO.getInstance().getBaseById(selection.getIdPersnl(),
					Tables.getTableName(Tables.TableName.personnel));
			Base grd = FichierBaseDAO.getInstance().getBaseById(selection.getIdGrd(),
			...
			emplName=selection.getNomPrenom();
	...
	private void searchParametre() {
		...
		if (parm != null) {
			...
			if (getAge() < getAgeMin()) {
				msgInfo = "Il faut avoir atteint l'age de retaraite (" + parm.getAgeRetraite()
						+ " ans) pour demander la prolongation !";
				disableSave = true;
			}
		}
	}
	...
	public void onEmployeSelected(SelectEvent event) {
		selection = (EmployeC) event.getObject();
		completeEmploye();
	...
	public void checkPeriod() {
		...
		if (parm != null) {
			if (getAge() < parm.getAgeRetraite()) {
				disableSave = true;
				HelperC.afficherAttention("Information", " L'Employé n'a pas encore l'age  de retraite ");
				return;
			}
			if (getAgeRetraiteDemande() > parm.getPeriodeProlongation()) {
				disableSave = true;
				HelperC.afficherAttention("Information",
						"On ne peut pas dépasser une prolongation de " + parm.getPeriodeProlongation() + " ans");
				return;
			}
			if (getAge() + getAgeRetraiteDemande() > parm.getAgeMaxRetraite()) {
				disableSave = true;
				HelperC.afficherAttention("Information",
						"On ne peut pas dépasser  " + parm.getAgeMaxRetraite() + " ans ");
				return;
			}
		} else {
			disableSave = true;
			HelperC.afficherAttention("Information",
					"Il faut paramétrer tous éléments en rapport la prologantion du travail et le personnel");
			return;
		}
	}
	...
	public void enregistrerDemande() {
		if (getEmploye() == null) {
			...
			setEtat(Constante
					.getEtatDemandeProlongationRetraite(Constante.EtatDemandeProlongationRetraite.demandeProlongation));
			setInLine(true);
			Historique hist = new Historique();
			hist.setDateOperation(Calendar.getInstance().getTime());
			hist.setOperateur(this.operateur);
			if (getId() == 0) {
				hist.setOperation("Création de la demande de prolongation de l'age de la retraite de l'employé "
						+ getEmploye().getNom() + " " + getEmploye().getPrenom());
			} else {
				hist.setOperation("Modification de la demande de prolongation de l'age de la retraite de l'employé "
						+ getEmploye().getNom() + " " + getEmploye().getPrenom());
			}
	...
	public void enregistrerDemandeLine() {
		if (getDecision() == 0 && getDateDecision() == null) {
			if (getEmploye() == null) {
				...
				setEtatProlongation(Constante.EtatDemandeProlongationRetraite.demandeProlongation);
				setEtat(Constante.getEtatDemandeProlongationRetraite(
						Constante.EtatDemandeProlongationRetraite.demandeProlongation));
	...
	public void enregistrerDecision() {
		if (getEmploye() == null) {
			...
			if (FactoryDAO.getInstance().insertUpdateDemandeProlongationRetraite(this)) {
				...
				if (parmGn != null) {
					if (getDecision() == 1)
						HelperC.sendEmail(parmGn.getSmtpServer(), parmGn.getMailOrigine(), parmGn.getMailOrigine(),
								parmGn.getPwdOrigine(), getEmploye().getEmail(),
								"Votre demande de prolongation de travail a été acceptée ",
								"Demande prolongation de travail");
					if (getDecision() == 2)
						HelperC.sendEmail(parmGn.getSmtpServer(), parmGn.getMailOrigine(), parmGn.getMailOrigine(),
								parmGn.getPwdOrigine(), getEmploye().getEmail(),
								"Votre demande de prolongation de travail a été refusée ",
								"Demande prolongation de travail.");
				}
				chargementDemande();
				clear();
	...
	public void supprimer() {
		if (getId() == 0) {
			...
			hist.setOperateur(this.operateur);
			if (getId() == 0) {
				hist.setOperation("Création de la demande de prolongation de l'âge de la retraite de l'employé "
						+ getEmploye().getNom() + " " + getEmploye().getPrenom());
			} else {
				hist.setOperation("Modification de la demande de prolongation de l'age de la retraite de l'employé "
						+ getEmploye().getNom() + " " + getEmploye().getPrenom());
