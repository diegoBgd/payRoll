   ...
   extends ParametrageFinCarriereC
 {
   ...
   private void init() {
     ...
     for (TypeNotationC typeNotation : FichierBaseDAO.getInstance().getAllTypeNotation()) {
       this.listTypeNotation.add(new SelectItem(Integer.valueOf(typeNotation.getId()), typeNotation.getDesignation()));
     }
   }
   ...
   public void save() {
     ...
     else if (FichierBaseDAO.getInstance().insertUpdateParametrageFinCarriere(this)) {
   ...
   public void supprimer() {
     try {
       ...
       else if (FichierBaseDAO.getInstance().delete(getId(), Tables.getTableName(Tables.TableName.parametrageFinCarriere))) {
