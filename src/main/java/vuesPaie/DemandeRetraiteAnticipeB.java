...
public void setMsgInfo(String msgInfo) {
   ...
   private void charger() {
     ...
     if (this.operateur == null || this.exercice == null) {
       ...
       if (frm != null) {
         ...
         setEtat(Constante.getEtatDemandeRetraiteAnticipe(Constante.EtatDemandeRetraiteAnticipe.traitementRetraite));
	...
	private void searchParametre(){
		...
		if(parm!=null)
		{
			...
			{
				disableSave=true;
        		HelperC.afficherAttention("Information", "L'employé doit avoir "+parm.getPeriodeAnticipe()+" ans d'ancienneté !");
        		msgInfo="Il faut avoir l'anciennété de "+parm.getPeriodeAnticipe()+" ans pour demander la retraite anticipée ";
        		return;
			}
		}
	}
   ...
   public void enregistrerDemande() {
     if (getEmploye() == null) {
       ...
       if (getId() == 0) {
         hist.setOperation("Création de la demande de la retraite anticipée de l'employé " + getEmploye().getNom() + " " + getEmploye().getPrenom());
       } else {
         hist.setOperation("Modification de la demande de la retraite anticipée  de l'employé " + getEmploye().getNom() + " " + getEmploye().getPrenom());
   ...
   public void enregistrerDecision() {
     ...
     if (FactoryDAO.getInstance().insertUpdateDemandeRetraiteAnticipe(this)) {
       ...
       HelperC.afficherMessage("Information", "Succès de l'opération");
				...
				{
				if(getDecision()==1)
				    HelperC.sendEmail(parmGn.getSmtpServer(), parmGn.getMailOrigine(), parmGn.getMailOrigine(), parmGn.getPwdOrigine(), getEmploye().getEmail(), "Votre demande de retraite anticipée a été accptée ", "Demande retraite anticipée");
				if(getDecision()==2)
				     HelperC.sendEmail(parmGn.getSmtpServer(), parmGn.getMailOrigine(), parmGn.getMailOrigine(), parmGn.getPwdOrigine(), getEmploye().getEmail(), "Votre demande de retraite anticipée a été refusée", "Demande retraite anticipée");
				}
      chargementTraite();
       clear();
     } else {
